      ...
      return (
        <div className="min-h-screen w-full bg-surface flex items-center justify-center p-margin-mobile">
          <div className="max-w-md w-full bg-surface-container-lowest border border-outline-variant p-8 md:p-10 shadow-2xl text-center space-y-6">
            <div className="w-16 h-16 bg-deep-space/10 text-deep-space rounded-full flex items-center justify-center mx-auto">
              <SmileySad className="text-3xl" weight="bold" />
            </div>
            
            <div className="space-y-2">
              <span className="font-label-caps text-xs text-accent tracking-widest uppercase block font-bold">
              ...
              {this.state.error && process.env.NODE_ENV !== 'production' && (
                <div className="mt-4 p-3 bg-red-950/80 border border-red-500/50 text-left rounded text-xs font-mono text-red-200 overflow-x-auto">
                  <div className="font-bold text-red-300">{this.state.error.name}: {this.state.error.message}</div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <button
                ...
                className="flex-1 bg-deep-space text-surface font-label-caps uppercase tracking-widest py-3 px-4 hover:bg-accent hover:text-deep-space transition-all border border-deep-space flex items-center justify-center gap-2 cursor-pointer font-bold"
              >
